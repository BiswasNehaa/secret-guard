...
def build_parser():
    ...
    baseline = subparsers.add_parser(
        "baseline",
        help="Scaffold a baseline file from the current findings.",
    )
    ...
    baseline.add_argument(
        "--output", default="secret-guard-baseline.json",
        metavar="FILE",
        help="Path to write the baseline file (default: %(default)s).",
    )
    baseline.add_argument(
...
def cmd_baseline(args):
    """Scan and write the resulting findings out as a baseline file.

    The baseline schema matches what --baseline/secret-guard.json's
    "baseline" key already accept: {"path", "rule_id", "hash"} entries,
    hashed the same way filter_baseline() hashes a finding's value, so a
    freshly generated baseline immediately suppresses every finding it
    was generated from.
    """
    ...
    for f in sorted(findings, key=lambda f: (f["path"], f["line"], f["rule_id"])):
        value_hash = hashlib.sha256(f["value"].encode("utf-8")).hexdigest()
        key = (f["path"], f["rule_id"], value_hash)
        if key in seen:
            continue
        seen.add(key)
        entries.append({"path": f["path"], "rule_id": f["rule_id"], "hash": value_hash})

    baseline_content = {
        "//": (
            "secret-guard baseline, generated by `secret-guard baseline`. "
            "Each entry suppresses one exact value at that path/rule; "
            "regenerate with --force after intentionally rotating or "
            "adding secrets."
        ),
        "baseline": entries,
    }
    ...
    print(f"Wrote {len(entries)} baseline entries to {args.output}")
    return 0
